...
type internal IntelliSenseAgent() =
  ...
  /// Invokes dot-completion request and writes information to the standard output
  member x.ToolTipCompletion(opts, ((line, column) as pos), lineStr, name, time) =
    try
      try
        // Get the long identifier before the current location
        // 'residue' is the part after the last dot and 'longName' is before
        ...
        | Some(info) ->
            // Get items & generate output
            let decls = info.GetDeclarations(pos, lineStr, (longName, residue), identToken)
            let decl = decls.Items |> Array.tryFind (fun i -> i.Name = name)

            if decl <> None then
                match decl.Value.DescriptionText with
                | DataTipText(elems) 
...
module internal CommandInput = 
  ...
  type Command =
    ...
    | ToolTipCompletion of Position * string * int option
    | GetErrors
    | Script of string 
    | Parse of bool
    | Error of string
  ...
  // Parse 'completion <line> <col> [timeout]' command
  let tipCompletion = parser {
    let! f = string "tipCompletion "
    let! line = some alphanum |> Parser.map (String.ofSeq >> int)
    let! _ = many (string " ")
    let! col = some alphanum |> Parser.map (String.ofSeq >> int)
    let! _ = many (string " ")
    let! name = some item |> Parser.map String.ofSeq
//    let! timeout = 
//      (parser { let! _ = some (string " ")
//                return! some alphanum |> Parser.map (String.ofSeq >> int >> Some) }) <|>
//      (parser { return None })
    return ToolTipCompletion((line, col), name, None) }
  ...
  let parseCommand input = 
    ...
    let cmds = errors <|> help <|> parse <|> script <|> tipCompletion <|> completionOrTip <|> error
...
module internal Main = 
  ...
  let rec main (state:State) =
    ...
    | ToolTipCompletion((line, column) as pos, name, timeout), Some(opts) ->
        // Trigger autocompletion (when we already loaded a file)
        if line >= state.Lines.Length || line < 0 then
          Console.Error.WriteLine("ERROR: Line is out of range")
        else
          agent.ToolTipCompletion(opts, pos, state.Lines.[line], name, timeout)
        main state
    ...
    | Parse _, None | Completion _, None | ToolTip _, None | ToolTipCompletion _, None ->
